...
struct EvaluatedAttack {
	inheirent_path: Rc<PathBuf>,
	check_ability: Ability,
	check_bonus: i32,
	check_bonuses: Vec<(i32, Rc<PathBuf>)>,
	damage_by_type: MultiMap<Option<DamageType>, (Roll, Rc<PathBuf>)>,
	modifiers: MultiMap<Modifier, Rc<PathBuf>>,
}

impl EvaluatedAttack {
	pub fn new(action: &Action, attack: &Attack, state: &Character) -> Self {
		let check_ability;
		let mut check_bonuses = Vec::<(i32, Rc<PathBuf>)>::new();
		let mut damage_by_type = MultiMap::<Option<DamageType>, (Roll, Rc<PathBuf>)>::new();
		let mut modifiers = MultiMap::<Modifier, Rc<PathBuf>>::new();
		...
		match &attack.check {
			...
			AttackCheckKind::SavingThrow { base, dc_ability, proficient, save_ability } => {
				check_ability = *save_ability;
				check_bonuses.push((*base, inheirent_path.clone()));
				if let Some(ability) = dc_ability {
					let modifier = state.ability_scores().get(*ability).score().modifier();
					let modifier_path = Rc::new(PathBuf::from(format!("{} modifier", ability.long_name())));
					check_bonuses.push((modifier, modifier_path));
				}
				if *proficient {
					check_bonuses.push((state.proficiency_bonus(), proficiency_path.clone()));
				}
		...
		// Find Atk Roll bonuses from derived data generated by mutators
		for (bonus, modifier, path) in state.attack_bonuses().get_weapon_attack(action) {
			let path = Rc::new(path.to_owned());
			check_bonuses.push((bonus, path.clone()));
			if let Some(modifier) = modifier {
				modifiers.insert(modifier, path.clone());
			}
		}
		// Find Damage bonuses from derived data generated by mutators
...
pub fn Actions() -> Html {
	...
	if selected_tags.contains(ActionTag::Attack) {
		...
		panes.push(html!(<div class="mb-2">
			...
			{features.into_iter().filter_map(|(feature_path, feature)| {
				...
				let onclick = open_feature_details.reform({
					let feature_path = feature_path.clone();
					move |_| feature_path.clone()
				});

				// Calculates Atk Roll & Damage bonuses based on the attack data, ability stat, and proficiency
				let mut evaluated = EvaluatedAttack::new(action, attack, &state);

				let check_bonus = evaluated.check_bonuses.iter().map(|(value, _)| *value).sum::<i32>();
				let check_bonus_tooltip = crate::data::as_feature_paths_html_custom(
					evaluated.check_bonuses.iter(),
					|(bonus, source)| (*bonus, source.as_path()),
					|bonus, path_str| {
						let bonus_sign = if bonus >= 0 { "+" } else { "-" };
						let bonus = bonus.abs();
						format!("<div>{bonus_sign}{bonus} ({path_str})</div>")
					}
				);

				let check_modifiers = evaluated.modifiers.iter_all().map(|(modifier, sources)| {
					let tooltip = crate::data::as_feature_paths_html(sources.iter().map(|rc| &**rc));
					html! {
						<Tooltip tag={"span"} content={tooltip} use_html={true}>
							<span aria-label={format!("{modifier:?}")} style="margin-left: 2px; display: block; height: 16px; width: 16px; vertical-align: middle; margin-top: -2px;">
								<RollModifier value={*modifier} />
							</span>
						</Tooltip>
					}
				}).collect::<Vec<_>>();

				Some(html!(<div class="border-bottom d-flex flex-row flex-wrap" {onclick}>
						<div class="d-flex flex-column flex-grow-1">
							{feature.name.clone()}
							{
								feature_path.parent()
								.map(crate::data::as_feature_path_text).flatten()
								.map(|text| html!(<div class="label">{text}</div>))
							}
						</div>
						<div class="d-flex flex-column">
							<div class="d-flex flex-row">
								{attack.kind.as_ref().map(|atk_kind| html! {
									<div class="d-flex flex-row align-items-baseline" style="font-size: 12px; min-width: 120px;">
										<span class="label me-1">{"Range:"}</span>
										{match atk_kind {
											AttackKindValue::Melee { reach } => html!(<>{reach}{"ft."}</>),
											AttackKindValue::Ranged { short_dist, long_dist, .. } => html!(<>{short_dist}{"ft."}{" / "}{long_dist}{"ft."}</>),
										}}
									</div>
								})}
								<Tooltip tag={"div"} content={check_bonus_tooltip} use_html={true}
									classes={"d-flex flex-row align-items-baseline"}
									style="font-size: 12px; min-width: 120px;"
								>
									{match attack.check {
										AttackCheckKind::AttackRoll {..} => html!(<>
											<span class="label me-1">{format!(
												"{} Roll:", evaluated.check_ability.long_name()
											)}</span>
											{if check_bonus >= 0 { "+" } else { "-" }}
											{check_bonus.abs()}
											{check_modifiers}
										</>),
										AttackCheckKind::SavingThrow { ..} => html!(<>
											<span class="label me-1">{"DC:"}</span>
											{evaluated.check_ability.abbreviated_name()}{" "}
											{if check_bonus >= 0 { "+" } else { "-" }}
											{check_bonus.abs()}
										</>),
									}}
								...
								{match &attack.damage {
									None => None,
									Some(DamageRoll { roll, base_bonus, damage_type }) => {
										if *base_bonus != 0 {
											evaluated.damage_by_type.insert(None, (Roll::from(*base_bonus), evaluated.inheirent_path.clone()));
										}
										...
										for (dmg_type_override, rolls_by_source) in evaluated.damage_by_type.into_iter() {
											...
											let roll_set = rolls_by_source.iter().map(|(roll, _)| *roll).collect::<RollSet>();
											let tooltip = crate::data::as_feature_paths_html_custom(
												rolls_by_source.iter(),
												|(roll, source)| (*roll, source.as_path()),
												|roll, path_str| format!("<div>{roll} ({path_str})</div>"),
											);
											let tooltip = format!("{}<br />{}", damage_type.display_name(), tooltip.unwrap_or_default());
											if !damage_rolls.is_empty() {
												damage_rolls.push(html!(" + "));
											}
											damage_rolls.push(html!(<Tooltip tag={"span"} content={tooltip} use_html={true} classes={"d-flex flex-row align-items-center"}>
												<span class="me-1">{roll_set.to_string()}</span>
												<DamageTypeGlyph value={damage_type} />
											</Tooltip>));
										}

										Some(html!(<div class="d-flex flex-row align-items-baseline" style="font-size: 12px; min-width: 120px;">
											<span class="label me-1">{"Damage:"}</span>
											{damage_rolls}
										</div>))
									}
								}}
							</div>
							{state.persistent().notes.get_first(&feature_path).map(|note| html! {
		...
		if let Some(attack) = &action.attack {
				...
				AttackCheckKind::AttackRoll { .. } => {
					attack_sections.push(html! {
						<div class="property">
							...
							{if evaluated.check_bonus >= 0 { "+" } else { "-" }}{evaluated.check_bonus}{";"}
							{check_bonus_list}
						</div>
					});
				}
			...
			if let Some(DamageRoll { roll, base_bonus, damage_type }) = &attack.damage {
				...
				for (dmg_type_override, rolls_by_source) in evaluated.damage_by_type.into_iter() {
					let damage_type = dmg_type_override.unwrap_or(*damage_type);
					let roll_set = rolls_by_source.iter().map(|(roll, _)| *roll).collect::<RollSet>();
					damage_rolls.push(html!(<div>
						<DamageTypeGlyph value={damage_type} />{" "}{damage_type.display_name()}
						<span class="ms-1">{roll_set.to_string()}</span>{";"}
						{rolls_by_source.iter().map(|(roll, source)| html!(<span class="ms-1">
							{roll.to_string()}
							<span style="color: var(--bs-gray-600);">
								{" ("}{crate::data::as_feature_path_text(source)}{")"}
							</span>
